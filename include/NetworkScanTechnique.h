...
class NetworkScanTechnique : public ITechnique {
public:
    NetworkScanTechnique(const std::string& name, const std::string& mitreId, const std::string& target);
    ~NetworkScanTechnique() = default;
...
private:
    std::string m_name, m_mitreId, m_target, m_result;
