...
class FileModificationTechnique : public ITechnique {
public:
    FileModificationTechnique(const std::string& name, const std::string& mitreId,
                               const std::string& file, const std::string& content);
    ~FileModificationTechnique() = default;
...
private:
    std::string m_name, m_mitreId, m_file, m_content, m_result;
